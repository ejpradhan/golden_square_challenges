...
RSpec.describe "integration" do
  context "when we add multiple tracks to the library" do
    it "comes back in the list" do
      ...
      track_2 = Track.new("my_title_1", "my_artist_2")
  ...
  context "with some tracks added" do
    it "it searches for those tracks by full title" do
      music_library = MusicLibrary.new
  ...
  it "searches for those tracks by partial title" do
    ...
    music_library.add(track_1)
    music_library.add(track_2)
    result = music_library.search_by_title("title_2") # searching for partial title
    expect(result).to eq [track_2]
  end

  context "where there are no tracks matching" do
    it "returns an empty list when searching" do
      music_library = MusicLibrary.new
      track_1 = Track.new("my_title_1", "my_artist_1")
      music_library.add(track_1)
      result = music_library.search_by_title("fred") # a keyword that doesn't exist
      expect(result).to eq []
    end
  end
end
